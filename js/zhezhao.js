...
function PDF_launch(ob, zzw, zzh, callback, titlestr) {
    ...
    var _docH = (d.all ? Math.max(d.body.clientHeight, d.documentElement.clientHeight) : window.innerHeight);
    var _docPH = (d.all ? Math.max(parent.d.body.clientHeight, parent.d.documentElement.clientHeight) : parent.window.innerHeight);
    var mainTop=parent.document.getElementsByClassName("main-content")[0].scrollTop;
    ...
    var titleHeight = 0;
    if (!titlestr) {
        PDF_c.innerHTML = "<a class='glyphicon glyphicon-remove' style='padding-top:5px;width:30px;height:30px;margin:-10px -18px 0 0;display:inline;position:relative;float:right;cursor:pointer;' onclick='PDF_close();'></a>";
    }
    else {
        if (titlestr == "no") {
            PDF_c.innerHTML = "";
    ...
    var ttt = _docPH > zzh ? _docPH - zzh : 100;
    if(mainTop+zzh>_docH){
        mainTop=_docH-zzh-ttt/2-10-10;
    }
    with (PDF_c.style) {
        width = zzw + 8 + "px";
        if (bodyHeight < zzh) {
    ...
    //var offset = (ttt / 2 + mainTop + 34 + titleHeight);
    with (PDF_i.style) {
        position = "absolute";
        width = zzw + "px";
        margin = "0px";
        ...
        height = zzh + "px";
        var ttt = _docPH > zzh ? _docPH - zzh : 100;
        top = (ttt / 2 + mainTop + 34 + titleHeight)+"px";
    ...
    //$(parent.frames["Iframe"].document).find(".main-content").animate({ scrollTop:"1000px" }, 0.1);
    //$(parent.frames["Iframe"].document).find("#PDF_bg_chezchenz").animate({ scrollTop:"1000px" }, 0.1);
    //$(parent.document).find("#PDF_bg_chezchenz").animate({ scrollTop:"1000px" }, 0.1);
    //console.log(parent.document.getElementsByClassName("main-content")[0].scrollTop);
    var tempTop=parent.document.getElementsByClassName("main-content")[0].scrollTop;
    $(parent.document).find(".main-content").animate({scrollTop:tempTop+"px" }, 0.01);
    //console.log(parent.document.getElementsByClassName("main-content")[0].scrollTop);
}
